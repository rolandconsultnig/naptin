...
import { notify } from '../utils/soundNotifications'
...
export function ChatPage() {
  ...
  const { socket, connected, sendMessage, joinChat, startTyping, stopTyping } = useSocket()
    ...
    const loadUsers = async () => {
      try {
        ...
        const usersList = response.data.map(u => ({
          ...
          avatar: u.profile_picture || `https://ui-avatars.com/api/?name=${u.username}&background=random`
  ...
  useEffect(() => {
    ...
    socket.on('receive_message', (message) => {
      ...
      if (!selectedChat || selectedChat.id !== message.sender_id) {
      ...
      setMessages((prev) => {
        ...
        if (!hasMessage && prev.length > 0) {
          ...
          for (let i = prev.length - 1; i >= 0; i--) {
            const m = prev[i]
            if (m.id >= 1000000000000 && m.content === message.content && m.sender_id === user.id) {
    ...
    socket.on('user_typing', (data) => {
      ...
      if (selectedChat && data.user_id === selectedChat.id) {
  ...
  const handleSendMessage = async (e) => {
    ...
    if (newMessage.trim() && selectedChat) {
      ...
      if (editingMessageId && editingMessageId < 1000000000000) { // Check if it's a database ID (not a timestamp)
        try {
          // Check if message exists in current messages
          const messageExists = messages.some(m => m.id === editingMessageId)
          if (messageExists) {
            await axios.put(`${getApiBase()}/messages/${editingMessageId}`, {
              content: messageContent
            }, { withCredentials: true })
          
            setMessages(prev => prev.map(m => 
              m.id === editingMessageId ? { ...m, content: messageContent } : m
            ))
          ...
          // Continue to send as new message if edit fails
        }
      }
      ...
      if (!connected) {
        ...
        console.error('Socket not connected:', { socket, connected })
        return
      }
      
      sendMessage(selectedChat.id, messageContent)
      // Don't show success toast here - wait for 'message_sent' event from server
    }
  }
  ...
  return (
      <div className="flex h-screen bg-[#e9edef]">
        ...
        <div className="bg-[#008069] px-4 py-3 flex items-center justify-between">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={handleEditProfile}>
            <img 
              ...
              onError={(e) => {
                e.target.src = `https://ui-avatars.com/api/?name=${user?.username}&background=00a884&color=fff`
              }}
            />
            <div>
          ...
          <>
            ...
            <div className="whatsapp-chat-bg flex-1 overflow-y-auto p-4">
              {(messageSearchTerm ? messages.filter(m => 
                ...
                <div
                  key={msg.id || index}
                  className={`flex ${msg.sender_id === user.id ? 'justify-end' : 'justify-start'} mb-2 group`}
                >
                  <div className={`relative ${
